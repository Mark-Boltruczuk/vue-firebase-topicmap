          ...
          <v-flex xs12 sm6>
            <v-card id="create">
              <v-speed-dial v-model="fab" :top="top" :bottom="bottom" :right="right" :left="left" :direction="direction"
                ...
                <v-tooltip bottom>
                  <v-btn :href="'/iframe/' + libraryId + '/topicmaps/' + topicMapId + '/hyper-tree'" target="_blank"
                  slot="activator" small dark fab color="pink"><v-icon>open_in_new</v-icon></v-btn>
                  <span>New Window</span>
                </v-tooltip>
                <v-tooltip bottom>
                  <v-btn slot="activator" small dark fab color="indigo" v-on:click.native="refresh"><v-icon>refresh</v-icon></v-btn>
                  <span>Refresh</span>
                </v-tooltip>
                ...
                <v-tooltip bottom v-if="userIsTeamMember">
                  <v-btn v-on:click.native="viewTopicmapUI('tree-list')"
                  slot="activator" small fab color="primary"><v-icon>list</v-icon></v-btn>
                  <span>TreeList</span>
                </v-tooltip>
                <v-tooltip bottom>
        ...
        <v-layout row wrap>
          <v-flex d-flex xs12 mb-0>
            <v-card class="blue-grey darken-2" dark>
              ...
              <v-card-text>Title: {{topicMap.name}}</v-card-text>
              <v-card-text>Description: {{topicMap.description}}</v-card-text>
              <v-card-text>Subject: {{topicMap.subject}}</v-card-text>
              <v-card-text>Tags: {{topicMap.tags}}</v-card-text>
            </v-card>
          </v-flex>
        </v-layout>
      </v-flex>
...
<script>
import { Hyperbolic } from "./hyperMap";
import { mapActions, mapGetters } from "vuex";

export default {
  props: ["topicMap", "topicMapId", "libraryId"],
  components: {
    "hyper-bolic": Hyperbolic
  },
  data() {
    return {
      isLoading: false,
